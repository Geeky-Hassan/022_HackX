...
import { User, LogOut, Settings2, ChevronLeft } from "lucide-react";
...
const SIDEBAR_ITEMS = [
  {
    ...
    label: "My Profile",
    href: "/mp/profile",
    isActive: (pathname: string) => pathname === "/mp/profile",
  },
  {
    icon: Settings2,
    label: "Preferences",
    href: "/mp/settings",
    isActive: (pathname: string) => pathname.includes("/mp/settings"),
  },
] as const;
  ...
  const renderSidebarItem = (item: (typeof SIDEBAR_ITEMS)[number], index: number) => {
    ...
    return (
      <li key={index} className={itemClasses}>
        <Link href={item.href} className="flex items-center gap-2 w-full" onClick={onClose}>
          {content}
        </Link>
      </li>
    );
    ...
    <aside className="md:p-2 h-screen overflow-hidden">
      <div className={sidebarClasses}>
        ...
        <div className="mt-[3.5rem] space-y-8">
          <header>
            <Link href={"/mp"} className="flex items-center gap-1 mb-2">
